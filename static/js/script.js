    ...
    fetch('/analyze', {
        ...
        .then(data => {
            ...
            data.tokens_list.forEach(token => {
                ...
                prCell.textContent = (['FOR', 'IF', 'DO', 'WHILE', 'ELSE', 'PROGRAMA', 'READ', 'INT', 'SYSTEM','OUT','PRINTLN'].includes(token.type)) ? 'X' : '';
                prCell.className = 'border border-black px-4 py-2 text-center';
                if (['FOR', 'IF', 'DO', 'WHILE', 'ELSE', 'PROGRAMA', 'READ', 'INT', 'SYSTEM','OUT','PRINTLN'].includes(token.type)) totalPR++;
                ...
                symbolCell.textContent = (token.type === 'LPAREN' || token.type === 'RPAREN' || token.type === 'SEMICOLON' || token.type === 'COMMA' || token.type === 'LBRACE' || token.type === 'RBRACE'  || token.type === 'LTE'|| token.type === 'PLUS' || token.type === 'DOT' || token.type === 'ASSIGN' ) ? 'X' : '';
                symbolCell.className = 'border border-black px-4 py-2 text-center';
                if (token.type === 'LPAREN' || token.type === 'RPAREN' || token.type === 'SEMICOLON' || token.type === 'COMMA' || token.type === 'LBRACE' || token.type === 'RBRACE' || token.type === 'LTE'|| token.type === 'PLUS' || token.type === 'DOT' || token.type === 'ASSIGN' ) totalSymbols++;
